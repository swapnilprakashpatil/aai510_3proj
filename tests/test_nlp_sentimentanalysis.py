...
def test_preprocess_text(nlp_sentiment):
    """Test text preprocessing"""
    text = "This is a TEST with Numbers 123 and Punctuation!!!"
    processed = nlp_sentiment.preprocess_text(text)
    
    assert processed == "this is a test with numbers and punctuation"
    
    # Test with non-string input
    assert nlp_sentiment.preprocess_text(None) == ""
    assert nlp_sentiment.preprocess_text(123) == ""

def test_emotion_detection(nlp_sentiment, sample_texts):
    """Test emotion detection functionality"""
    results = nlp_sentiment.detect_emotions(sample_texts)
...
def test_bert_loading(nlp_sentiment):
    """Test loading the TinyBERT model"""
    success = nlp_sentiment.load_bert_model()
    assert success
    assert nlp_sentiment.tokenizer is not None
    assert nlp_sentiment.model is not None

@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
def test_gpu_usage(nlp_sentiment):
    """Test that model uses GPU if available"""
    nlp_sentiment.device = 'cuda'
    nlp_sentiment.load_bert_model()
    assert nlp_sentiment.model.device.type == 'cuda'

def test_data_splitting(nlp_sentiment, sample_texts, sample_labels):
    """Test train/val/test split functionality"""
    # Set a fixed random state for reproducibility
    train_texts, val_texts, test_texts, train_labels, val_labels, test_labels = \
        nlp_sentiment.train_test_split(sample_texts, sample_labels, test_size=0.33, val_size=0.25, random_state=42)
...
def test_metrics_calculation(nlp_sentiment):
    ...
    nlp_sentiment.metrics = {'accuracy': [metrics['accuracy']], 'precision': [metrics['precision']], 
                          'recall': [metrics['recall']], 'f1': [metrics['f1']]}
    
    stored_metrics = nlp_sentiment.get_metrics()
...
def test_analyze_patient_sentiment(nlp_sentiment, sample_texts):
    """Test the analyze_patient_sentiment method"""
    # Mock sentiment prediction function to return predetermined labels
    nlp_sentiment.predict_sentiment = lambda texts: [0, 1, 0, 1, 1, 0]
    
    # Call the method
    results = nlp_sentiment.analyze_patient_sentiment(sample_texts)
